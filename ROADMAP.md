...
- Historical Orin Triton finding (2026-06-01): `nvinferserver` was installed and usable under the prior image. NVIDIA's Triton YOLOv3 ONNX sample was staged at `/home/roger/deepstream-models/triton-yolov3`, completed against the 20-second marathon clip, and emitted KITTI detections. It averaged only about `4.4 FPS`, so it proved the `nvinferserver`/metadata path but was not field-ready. Next work should focus on a faster TensorRT-native broad YOLO detector pack rather than reviving the failed repo-local YOLOv5 custom-parser experiment.
- Current JetPack 7 migration finding (2026-08-14): the Orin now runs JetPack 7.2.1/L4T 39.2.1, DeepStream 9.1, TensorRT 10.16, and CUDA 13.2. Native `nvinfer`, Jetson encode/decode, and NvDCF assets are present; Triton is not installed, so the old `nvinferserver` proof is historical rather than an event dependency. TensorRT engines must be rebuilt and versioned for the installed runtime.
- Current replay-regression finding (2026-08-14): the first recall-first JetPack 7 baseline on real marathon/NORC footage fails the intended field gate. Preserve that result as evidence and improve association/transit scoring rather than weakening the 95% recall / 15% negative-promotion thresholds. Tesseract also fails the identifier rollout gate and remains opt-in.
